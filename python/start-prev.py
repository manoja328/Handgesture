...
#textbelow=['one','Jha','three','Rock on!','punch you!','palm open','Cick Mouse']
...
grayimage = cv.CreateImage((640, 480), 8, 1)
mascara = cv.CreateImage((640,480), 8, 1)
cinza = cv.CreateImage((640,480), 8, 1)

for i in xrange(5):
    primeiraImagem = cv.QueryFrame(capture)
    cv.Flip(primeiraImagem,primeiraImagem,1)
    fundo = cv.CloneImage(primeiraImagem)

fundogray = cv.CreateImage((640, 480), cv.IPL_DEPTH_8U, 1)
cv.CvtColor(fundo, fundogray, cv.CV_BGR2GRAY)
cv.EqualizeHist(fundogray,fundogray)
imagegray = cv.CreateImage((640, 480), cv.IPL_DEPTH_8U, 1)
...
while True:
    ...
    cv.CvtColor(image,imagegray, cv.CV_BGR2GRAY)
    cv.EqualizeHist(imagegray,imagegray)
    
    cv.AbsDiff(imagegray,fundogray,mascara)
    #cv.CvtColor(mascara, cinza, cv.CV_BGR2GRAY)
    
    cv.Threshold(mascara,skin, 50,255,cv.CV_THRESH_BINARY)
